...
onAuthStateChanged(auth, (user) => {
  if (user) {
    ...
    getDoc(docRef).then((snapshot) => { 
      ...
      if(emotion == "happy")
      {
          imageUrls = docData.happy_images
          document.getElementById("friend-name").textContent = "Happy Memories!";
      }
      if(emotion == "sad")
      {
          imageUrls = docData.sad_images
          document.getElementById("friend-name").textContent = "Sad Memories!";
      }
      if(emotion == "surprised")
      {
          imageUrls = docData.surprised_images
          document.getElementById("friend-name").textContent = "Surprised Memories!";
      }
      if(emotion == "angry")
      {
          imageUrls = docData.angry_images
          document.getElementById("friend-name").textContent = "Angry Memories!";
      }
...
async function addImage(imageUrl)
{
  ...
  outerdivone.setAttribute("style", "max-height: 250px; overflow: hidden; margin-bottom: 30px;");
